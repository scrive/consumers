-- | Consumer runtime: the three daemon threads that drive a running consumer.
--
...
--   transaction; the results are folded back into a single batched @UPDATE@
--   per dispatch cycle.
--
-- The @spawn*@ functions are exposed for testing and instrumentation; normal
-- users should call 'runConsumer'.
